(() => {
  ...
  function render(){const host=document.getElementById("tab-pattern-knowledge");if(!host)return;let box=document.getElementById("st40VisualAssetPolicies");if(!box){box=document.createElement("div");box.id="st40VisualAssetPolicies";host.prepend(box)}const html=`<div class="card" style="margin-bottom:14px"><div class="brand">VISUAL ASSET POLICY</div><div class="small muted" style="margin-top:6px">По умолчанию производные визуалы живут только в локальном кэше и восстанавливаются из рецепта. Материнским и специальным паттернам можно разрешить постоянный серверный ассет.</div></div>${items.map(x=>rowHtml(x)).join("")}${rowHtml({tier:"special",imagePolicy:"cache_only"},true)}`;if(box.innerHTML===html)return;box.innerHTML=html;box.querySelectorAll("[data-va-row]").forEach(row=>row.querySelector("[data-va-save]")?.addEventListener("click",()=>save(row)))}
  document.getElementById("refreshBtn")?.addEventListener("click",()=>load());
  document.querySelector('[data-tab="pattern-knowledge"]')?.addEventListener("click",()=>load());
  setTimeout(load,500);
  window.__SETKA_ADMIN_VISUAL_ASSETS_V40__={version:2,load,render,get items(){return items.slice()}};
})();