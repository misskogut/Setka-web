...
await p1.evaluate(()=>window.FoundationV01.finishSynthetic('passed',{smoke:'phase1'}));await p1.close();
...
await p2.click(`[data-pattern-id="${id}"]`);await p2.waitForSelector('#viewer:not(.hidden)');await p2.click('#heartButton');s=await p2.evaluate(()=>window.FoundationV01.state());if(!!s.patterns.find(x=>x.patternId===id).favorite!==initial)throw new Error('favorite_restore_failed');await p2.evaluate(()=>window.FoundationV01.finishSynthetic('passed',{smoke:'phase2',restored:true}));
console.log(JSON.stringify({ok:true,patterns:s.patterns.map(x=>x.patternId),favoritePersisted:true,restored:true}));await p2.close();await browser.close();