...
OPEN_URL="${URL}?front=B2.7.1"
HEALTH="${URL}api/b2/health"
EXPECTED_HEALTH="SETKA_LOCAL_FRONT_B271_READY"

mkdir -p "$FRONT_DIR"

echo "SETKA LOCAL FRONT · preparing B2.7.1 projection workspace over B2.5 data context"
refresh_file() {
...
refresh_file "$OVERLAY_B24_URL" "$OVERLAY_B24" && echo "SETKA LOCAL FRONT · B2.4 trace layer refreshed" || echo "SETKA LOCAL FRONT · B2.4 trace cache preserved"
refresh_file "$OVERLAY_B25_URL" "$OVERLAY_B25" && echo "SETKA LOCAL FRONT · B2.5 event-context layer refreshed" || echo "SETKA LOCAL FRONT · B2.5 event-context cache preserved"
refresh_file "$OVERLAY_B26_URL" "$OVERLAY_B26" && echo "SETKA LOCAL FRONT · B2.6 workspace movement layer refreshed" || echo "SETKA LOCAL FRONT · B2.6 workspace cache preserved"
refresh_file "$OVERLAY_B27_URL" "$OVERLAY_B27" && echo "SETKA LOCAL FRONT · B2.7 projection workspace refreshed" || echo "SETKA LOCAL FRONT · B2.7 projection workspace cache preserved"
refresh_file "$OVERLAY_B271_URL" "$OVERLAY_B271" && echo "SETKA LOCAL FRONT · B2.7.1 runtime projection layer refreshed" || echo "SETKA LOCAL FRONT · B2.7.1 runtime layer cache preserved"
...
refresh_file "$BRIDGE_B25_URL" "$BRIDGE_B25" && echo "SETKA LOCAL FRONT · B2.5 bridge refreshed" || echo "SETKA LOCAL FRONT · B2.5 bridge cache preserved"
refresh_file "$BRIDGE_B27_URL" "$BRIDGE" && echo "SETKA LOCAL FRONT · B2.7.1 runtime bridge refreshed" || echo "SETKA LOCAL FRONT · B2.7.1 runtime bridge cache preserved"
...
if [ -f "$OVERLAY_B23" ] && [ -f "$OVERLAY_B24" ] && [ -f "$OVERLAY_B25" ] && [ -f "$OVERLAY_B26" ] && [ -f "$OVERLAY_B27" ] && [ -f "$OVERLAY_B271" ]; then
  ...
  echo "SETKA LOCAL FRONT · B2.7.1 combined overlay ready"
fi

if [ ! -f "$TARGET" ] || [ ! -f "$OVERLAY" ] || [ ! -f "$BRIDGE_B23" ] || [ ! -f "$BRIDGE_B24" ] || [ ! -f "$BRIDGE_B25" ] || [ ! -f "$BRIDGE" ]; then
  echo "STOP · SETKA LOCAL FRONT · B2.7.1 files incomplete"
  exit 2
fi

if command -v curl >/dev/null 2>&1 && curl -fsS --max-time 2 "$HEALTH" 2>/dev/null | grep -q "$EXPECTED_HEALTH"; then
  echo "SETKA LOCAL FRONT · ALREADY RUNNING · B2.7.1 UI / B1 DATA · $URL"
...
echo "SETKA LOCAL FRONT · STARTING B2.7.1 UI / B1 DATA · $URL"
