...
OPEN_URL="${URL}?front=B2.6"
HEALTH="${URL}api/b2/health"
EXPECTED_HEALTH="SETKA_LOCAL_FRONT_B25_READY"

mkdir -p "$FRONT_DIR"

echo "SETKA LOCAL FRONT · preparing B2.6 workspace over B2.5 data bridge"
refresh_file() {
...
if [ -f "$OVERLAY_B23" ] && [ -f "$OVERLAY_B24" ] && [ -f "$OVERLAY_B25" ] && [ -f "$OVERLAY_B26" ]; then
  ...
  echo "SETKA LOCAL FRONT · B2.6 combined overlay ready"
fi

if [ ! -f "$TARGET" ] || [ ! -f "$OVERLAY" ] || [ ! -f "$BRIDGE_B23" ] || [ ! -f "$BRIDGE_B24" ] || [ ! -f "$BRIDGE" ]; then
  echo "STOP · SETKA LOCAL FRONT · B2.6 files incomplete"
  exit 2
fi

if command -v curl >/dev/null 2>&1 && curl -fsS --max-time 2 "$HEALTH" 2>/dev/null | grep -q "$EXPECTED_HEALTH"; then
  echo "SETKA LOCAL FRONT · ALREADY RUNNING · B2.6 UI / B2.5 DATA · $URL"
...
echo "SETKA LOCAL FRONT · STARTING B2.6 UI / B2.5 DATA · $URL"
