...
OVERLAY_URL="$BASE_URL/setka-front-b23.js"
BRIDGE_URL="$BASE_URL/setka-front-bridge-b23.py"
...
EXPECTED_HEALTH="SETKA_LOCAL_FRONT_B23_READY"

mkdir -p "$FRONT_DIR"

echo "SETKA LOCAL FRONT · preparing B2.3"
refresh_file() {
...
refresh_file "$OVERLAY_URL" "$OVERLAY" && echo "SETKA LOCAL FRONT · B2.3 overlay refreshed" || echo "SETKA LOCAL FRONT · overlay cache preserved"
refresh_file "$BRIDGE_URL" "$BRIDGE" && echo "SETKA LOCAL FRONT · B2.3 bridge refreshed" || echo "SETKA LOCAL FRONT · bridge cache preserved"
chmod 700 "$BRIDGE" 2>/dev/null || true

if [ ! -f "$TARGET" ] || [ ! -f "$OVERLAY" ] || [ ! -f "$BRIDGE" ]; then
  echo "STOP · SETKA LOCAL FRONT · B2.3 files incomplete"
  exit 2
fi

if command -v curl >/dev/null 2>&1 && curl -fsS --max-time 2 "$HEALTH" 2>/dev/null | grep -q "$EXPECTED_HEALTH"; then
  echo "SETKA LOCAL FRONT · ALREADY RUNNING · B2.3 · $URL"
...
if command -v lsof >/dev/null 2>&1 && lsof -nP -iTCP:"$PORT" -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  if printf '%s' "$CMD" | grep -Eq "python3 -m http.server $PORT|setka-front-bridge.py"; then
    echo "SETKA LOCAL FRONT · upgrading existing SETKA localhost process to B2.3"
...
echo "SETKA LOCAL FRONT · STARTING B2.3 · $URL"
