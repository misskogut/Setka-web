...
OPEN_URL="${URL}?front=B2.7.3"
HEALTH="${URL}api/b2/health"
EXPECTED_HEALTH="SETKA_LOCAL_FRONT_B273_READY"

mkdir -p "$FRONT_DIR"

echo "SETKA LOCAL FRONT · preparing B2.7.3 safe workbench / DATA B1"
...
refresh_file "$OVERLAY_B26_URL" "$OVERLAY_B26" && echo "PASS · B2.6 legacy movement substrate" || echo "HOLD · B2.6 cache"
refresh_file "$OVERLAY_B27_URL" "$OVERLAY_B27" && echo "PASS · B2.7 projection bindings" || echo "HOLD · B2.7 cache"
refresh_file "$OVERLAY_B271_URL" "$OVERLAY_B271" && echo "PASS · B2.7.1 runtime bridge UI" || echo "HOLD · B2.7.1 cache"
refresh_file "$OVERLAY_B273_URL" "$OVERLAY_B273" && echo "PASS · B2.7.3 safe workbench" || echo "HOLD · B2.7.3 cache"
...
refresh_file "$BRIDGE_B27_URL" "$BRIDGE" && echo "PASS · bridge B2.7.3" || echo "HOLD · bridge B2.7.3 cache"
...
if [ -f "$OVERLAY_B23" ] && [ -f "$OVERLAY_B24" ] && [ -f "$OVERLAY_B25" ] && [ -f "$OVERLAY_B26" ] && [ -f "$OVERLAY_B27" ] && [ -f "$OVERLAY_B271" ] && [ -f "$OVERLAY_B273" ]; then
  cat "$OVERLAY_B23" "$OVERLAY_B24" "$OVERLAY_B25" "$OVERLAY_B26" "$OVERLAY_B27" "$OVERLAY_B271" "$OVERLAY_B273" > "$OVERLAY.tmp" && mv "$OVERLAY.tmp" "$OVERLAY"
  chmod 600 "$OVERLAY" 2>/dev/null || true
  echo "PASS · B2.7.3 combined overlay ready"
fi

if [ ! -f "$TARGET" ] || [ ! -f "$OVERLAY" ] || [ ! -x "$BRIDGE" ]; then
  echo "STOP · SETKA LOCAL FRONT · B2.7.3 files incomplete"
  exit 2
fi

if command -v curl >/dev/null 2>&1 && curl -fsS --max-time 2 "$HEALTH" 2>/dev/null | grep -q "$EXPECTED_HEALTH"; then
  echo "SETKA LOCAL FRONT · ALREADY RUNNING · B2.7.3 · $URL"
...
echo "SETKA LOCAL FRONT · STARTING B2.7.3 UI / B1 DATA · $URL"
