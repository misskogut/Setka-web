...
echo "SETKA LOCAL FRONT · preparing B2.5"
refresh_file() {
...
refresh_file "$BRIDGE_B25_URL" "$BRIDGE" && echo "SETKA LOCAL FRONT · B2.5 bridge refreshed" || echo "SETKA LOCAL FRONT · B2.5 bridge cache preserved"
...
if command -v lsof >/dev/null 2>&1 && lsof -nP -iTCP:"$PORT" -sTCP:LISTEN >/dev/null 2>&1; then
  ...
  if printf '%s' "$CMD" | grep -Eq "python3 -m http.server $PORT|setka-front-bridge.py"; then
    echo "SETKA LOCAL FRONT · upgrading existing SETKA localhost process to B2.5"
...
echo "SETKA LOCAL FRONT · STARTING B2.5 · $URL"
