...
async function enter(){const d=await call('admin_snapshot');renderSnapshot(d.snapshot);showApp();setStatus('Вход подтверждён. Аварийный шлюз готов.','ok');}
async function login(){setStatus('Проверяю вход…');const id=$('presidentId').value.trim().toUpperCase();const key=$('presidentKey').value.trim();try{const d=await call('president_login',{setkaId:id,presidentKey:key});session=d.sessionToken;try{localStorage.setItem(SESSION_KEY,session)}catch{}await enter();}catch(e){session='';try{localStorage.removeItem(SESSION_KEY)}catch{}setStatus(e.message==='invalid_credentials'?'Неверный SETKA ID или ключ.':`Ошибка входа: ${e.message}`,'error');}}
async function restore(){try{session=localStorage.getItem(SESSION_KEY)||''}catch{}if(!session)return;try{await enter()}catch{session='';try{localStorage.removeItem(SESSION_KEY)}catch{}showLogin();}}
function enterMother(){if(!session){setStatus('Сначала нужен подтверждённый президентский вход.','error');return;}setStatus('Переход внутрь материнского SETKA…','ok');location.assign('foundation-president.html?ingress=recovery&build=018-yandex-unfreeze1');}
