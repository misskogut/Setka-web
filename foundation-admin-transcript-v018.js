...
function detailHtml(e){const parts=[];parts.push(`<span><b>№ записи:</b> ${esc(e.eventNo??'—')}</span>`);if(e.entityType||e.entityRef)parts.push(`<span><b>Сущность:</b> ${esc(e.entityType||'—')}${e.entityRef?` · ${esc(e.entityRef)}`:''}</span>`);if(e.version)parts.push(`<span><b>Версия:</b> ${esc(e.version)}</span>`);if(e.actorIdentityId)parts.push(`<span><b>Автор/исполнитель:</b> ${esc(actor(e))}</span>`);parts.push(`<span><b>Время события:</b> ${esc(time(e.occurredAt))}</span>`);parts.push(`<span><b>Записано в стенограмму:</b> ${esc(time(e.recordedAt))}</span>`);return parts.join('')}
function render(){
 ...
 <div class="transcriptMeta018"><span>${state.events.length} событий загружено · № ${esc(first)}–${esc(last)}</span><span>порядок: по № записи</span></div>
 ${state.hasMore?'<button id="transcriptOlder018" class="transcriptOlder018">↑ Загрузить более ранние номера</button>':''}
...
window.FoundationSystemTranscriptV018={version:'0.1.8',order:'event_no',refresh:()=>load(true),events:()=>state.events.slice()};
})();