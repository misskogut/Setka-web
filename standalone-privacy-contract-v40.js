(() => {
  ...
  const replacements=[
    ...
    [CURRENT_V2_CONSENT,NEW_CONSENT],
  ...
  async function renderStatus(){
    ...
    let cacheCopy="Визуальные превью создаются на устройстве и могут быть очищены без потери заметок или конфигураций: постоянным слепком остаётся кодовая капсула.";
    try{const s=await window.__SETKA_VISUAL_CACHE_V40__?.stats?.();if(s?.available)cacheCopy=`Визуальный кэш: ${Number(s.items)||0} превью · ${Math.round((Number(s.bytes)||0)/1024/1024*10)/10} МБ. Его можно очистить без потери данных: кодовые капсулы сохраняют рецепт визуала.`}catch(_){}
    let html;
  ...
  window.__SETKA_PRIVACY_CONTRACT_V40__={version:6,consentVersion:NEW_VERSION,mode:"private-system-public-separated",visuals:"capsule-plus-device-cache-plus-retained-public-preview"};
})();