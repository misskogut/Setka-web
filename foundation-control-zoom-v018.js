...
if(!frame)return;
let scale=1,pinch=null,raf=0,resizeObserver=null;
...
function ensureWorkspace(d){const shell=d.getElementById('app')||d.querySelector('.shell');if(!shell)return null;let wrap=d.getElementById('foundationWorkspace018');if(!wrap){wrap=d.createElement('div');wrap.id='foundationWorkspace018';shell.parentNode?.insertBefore(wrap,shell);wrap.appendChild(shell)}shell.style.width=`${WORKSPACE_WIDTH}px`;shell.style.minWidth=`${WORKSPACE_WIDTH}px`;shell.style.maxWidth='none';wrap.style.position='relative';return{shell,wrap}}
function rawHeight(shell){const z=shell.style.zoom;shell.style.zoom='1';const h=Math.max(shell.scrollHeight,shell.offsetHeight,900);shell.style.zoom=z;return h}
function updateExtent(workspace){if(!workspace)return;const {shell,wrap}=workspace;const h=rawHeight(shell);wrap.style.width=`${Math.ceil(WORKSPACE_WIDTH*scale)}px`;wrap.style.height=`${Math.ceil(h*scale)}px`;wrap.style.minHeight=`${Math.ceil(900*scale)}px`}
function apply(save=true){let d;try{d=frame.contentDocument}catch{return}if(!d)return;const workspace=ensureWorkspace(d);if(!workspace)return;const {shell}=workspace,root=d.documentElement,body=d.body;if(!root||!body)return;
 shell.style.transform='none';shell.style.transformOrigin='';shell.style.zoom=String(scale);
 root.style.overflow='auto';body.style.overflow='auto';root.style.scrollBehavior='auto';body.style.scrollBehavior='auto';root.style.overscrollBehavior='contain';body.style.overscrollBehavior='contain';root.dataset.shellZoom=String(scale);root.dataset.shellZoomMode='css-zoom-hit-safe';
 updateExtent(workspace);if(save)persist();
 try{frame.contentWindow.dispatchEvent(new CustomEvent('foundation:admin-zoom',{detail:{scale,source:'gesture-canvas-css-zoom'}}))}catch{}
}
function schedule(save=false){cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>apply(save))}
function bindDoc(){let d;try{d=frame.contentDocument}catch{return}if(!d||d.documentElement.dataset.gestureZoomBound==='3')return;d.documentElement.dataset.gestureZoomBound='3';const scroller=()=>d.scrollingElement||d.documentElement;
...
window.FoundationControlZoomV018={get:()=>scale,set:v=>{scale=clamp(v);apply(true);return scale},reset:()=>{scale=1;apply(true);return scale},fit:()=>{let w=390;try{w=frame.clientWidth||innerWidth}catch{}scale=clamp((w-12)/WORKSPACE_WIDTH);apply(true);return scale},mode:'stable-canvas-css-zoom',workspaceWidth:WORKSPACE_WIDTH};
})();