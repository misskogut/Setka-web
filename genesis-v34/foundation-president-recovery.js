...
async function enter(){const d=await rawCall('admin_snapshot',{},session);persistSystemSession(session);renderSnapshot(d.snapshot);showApp();setStatus('Президент подтверждён. Активна единая сессия всей SETKA.','ok');}
...
function enterMother(){if(!session){setStatus('Сначала нужен подтверждённый Президентский вход.','error');return;}persistSystemSession(session);setStatus('Открываю живую SETKA на сегодняшнем состоянии…','ok');location.assign('setka-president-current.html?from=president-entry');}
