...
function captureModalPin(e){if(!modalPinMode)return;const modal=openPinCard();if(!modal||!modal.contains(e.target)||e.target.closest('.modalPinTool018,.modalPinComposer018'))return;const card=e.target.closest('.modalCard');if(!card)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();modalPinPending=describeModalPoint(e,modal);modalPinMode=false;updateModalPinUi();q('modalPinComposerMeta018').textContent=modalPinPending.targetLabel;q('modalPinComposer018').classList.add('open');setTimeout(()=>q('modalPinComment018')?.focus(),40)}
async function saveModalPin(){const p=modalPinPending,comment=q('modalPinComment018')?.value.trim();if(!p||!comment)return;const b=q('modalPinSave018');if(b)b.disabled=true;try{await control('pin_create',{...p,comment});closeModalPinComposer();window.FoundationControlPatchV016?.reloadPins?.();setTimeout(()=>window.FoundationPinOriginV018?.decorate?.(),350)}catch(e){alert('Не удалось сохранить PIN: '+e.message)}finally{if(b)b.disabled=false}}
