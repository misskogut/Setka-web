(() => {
  ...
  function stale(item){return !item?.snapshotDataUrl || !item?.snapshotSignature || item.snapshotSignature !== item.renderSignature}
  ...
  async function materialize(force=false){
    ...
    try{
      ...
      const targets=force?items:items.filter(stale);
      last={total:items.length,stored:items.length-targets.length,pending:targets.length};
      status(`Актуально ${last.stored} из ${last.total}.`,targets.length?`Материализую ${targets.length} отсутствующих или устаревших стартовых кадров…`:"Все стартовые слепки соответствуют текущим капсулам.");
      ...
      for(const item of targets){
        ...
        stored++;
        status(`Актуально ${stored} из ${items.length}.`,`Последний: ${item.patternId}`);
      }
      const after=await post({action:"mother-capsules"}),all=after.items||[],ready=all.filter(x=>!stale(x)).length;
      last={total:all.length,stored:ready,pending:all.length-ready};
      completed=last.pending===0;
      status(`Актуально ${last.stored} из ${last.total}.`,completed?"Капсула + серверный WebP готовы для всех стартовых паттернов.":`Осталось ${last.pending}.`);
      return completed;
  ...
  window.__SETKA_ADMIN_MOTHER_SNAPSHOTS_V40__={version:2,materialize,get state(){return{running,completed,last}}};
})();