...
let restoring=false,lastContext='',pinKinds=new Map();
...
function restorePresident(context,attempt=0){try{const d=frame.contentDocument;if(!d||!d.body)return retry();const key=context||'users',b=d.querySelector(`.nav button[data-page="${CSS.escape(key)}"]`);if(b){if(!b.classList.contains('active'))b.click();store(key);note('');return done()}if(attempt<18)return setTimeout(()=>restorePresident(context,attempt+1),100);const fallback=d.querySelector('.nav button.active')?.dataset?.page||'users';store(fallback);note(`В ${select?.value||'этой версии'} вкладки «${key}» ещё нет · открыт ${fallback}`);done()}catch{retry()}
 function retry(){if(attempt<18)setTimeout(()=>restorePresident(context,attempt+1),100);else done()}
}
function restoreUser(context,attempt=0){try{const d=frame.contentDocument;if(!d||!d.body)return retry();if(context?.startsWith('pattern:')){const id=context.slice(8),card=d.querySelector(`[data-pattern-id="${CSS.escape(id)}"]`);if(card){card.click();store(context);note('');return done()}if(attempt<24)return setTimeout(()=>restoreUser(context,attempt+1),120);store('patterns');note(`Паттерна «${id}» в ${select?.value||'этой версии'} нет · открыта библиотека`);return done()}store('patterns');note('');done()}catch{retry()}
 function retry(){if(attempt<24)setTimeout(()=>restoreUser(context,attempt+1),120);else done()}
}
function done(){restoring=false;setTimeout(capture,120);setTimeout(recolorPins,180)}
...
function recolorPins(){document.querySelectorAll('.v016PinMarker[data-pin-code]').forEach(m=>m.classList.toggle('syntheticAuthor',pinKinds.get(m.dataset.pinCode)==='synthetic'))}
...
setInterval(()=>{capture();recolorPins()},700);setTimeout(()=>{capture();fetchPinKinds();applyActorMode()},350);
window.FoundationContextV017={version:'0.1.7',current,restore,capture,key:PAGE_KEY};
})();