...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(action==="synthetic_login"){
    const personaKey=text(body.personaKey,120),persona=await rpc("foundation_resolve_synthetic_persona",{p_persona_key:personaKey});
    if(!persona?.identityId)return json({error:"unknown_synthetic",pairVersion:PAIR_VERSION},404);
    const sess=await makeSession(persona.identityId,2),run=await rpc("foundation_start_synthetic_run",{p_identity_id:persona.identityId,p_persona_key:personaKey}),snap=await rpc("foundation_user_snapshot_v014",{p_identity_id:persona.identityId});
  ...
  if(action==="favorite_set"){
    const s=await userSession(req,body);if(!s)return json({error:"invalid_user_session",pairVersion:PAIR_VERSION},401);const runId=s.snapshot?.profile?.synthetic?text(body.runId,80)||null:null;
    const result=await rpc("foundation_set_favorite",{p_identity_id:s.identity_id,p_pattern_id:text(body.patternId,80),p_value:Boolean(body.value),p_run_id:runId,p_metadata:{source:s.snapshot.profile.synthetic?"synthetic_front":"human_front",pairVersion:PAIR_VERSION}}),snapshot=await rpc("foundation_user_snapshot_v014",{p_identity_id:s.identity_id});
    return json({ok:true,pairVersion:PAIR_VERSION,result,snapshot});
  }
 ...
 }catch(e){console.error("setka-foundation-v014",e);return json({error:"server_error",detail:String(e?.message||e),pairVersion:PAIR_VERSION},500)}
});