...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(action==="user_login"){
    ...
    let snap;try{snap=await rpc("foundation_user_snapshot",{p_identity_id:row.identity_id})}catch{return json({error:"not_foundation_user",pairVersion:PAIR_VERSION},403)}
  ...
  if(action==="admin_attach_synthetics"){
    const s=await presidentSession(req,body);if(!s)return json({error:"invalid_president_session",pairVersion:PAIR_VERSION},401);
    return json({pairVersion:PAIR_VERSION,result:await rpc("foundation_attach_existing_synthetics",{p_session_hash:s.hash})});
  }
  if(action==="admin_snapshot"){
    const s=await presidentSession(req,body);if(!s)return json({error:"invalid_president_session",pairVersion:PAIR_VERSION},401);
    return json({ok:true,pairVersion:PAIR_VERSION,snapshot:await rpc("foundation_admin_snapshot",{p_session_hash:s.hash})});
  }
  ...
  if(action==="logout"){
    ...
    await rpc("foundation_record_event",{p_identity_id:s.identity_id,p_event_type:"logout",p_pattern_id:null,p_run_id:null,p_metadata:{source:"human_front",pairVersion:PAIR_VERSION}}).catch(()=>{});
 ...
 }catch(e){console.error("setka-foundation-v012",e);return json({error:"server_error",detail:String(e?.message||e),pairVersion:PAIR_VERSION},500)}
});